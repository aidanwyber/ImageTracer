import './styles.css';
import sampleImageUrl from '../test/test-1080.png?url';
...
function extractPalette(imageData, step) {
        ...
        for (let i = 0; i < data.length; i += 4) {
                ...
                const r = Math.min(255, Math.round(data[i] / roundedStep) * roundedStep);
                const g = Math.min(255, Math.round(data[i + 1] / roundedStep) * roundedStep);
                const b = Math.min(255, Math.round(data[i + 2] / roundedStep) * roundedStep);
                const key = `${r},${g},${b}`;
                if (!palette.has(key)) {
...
function traceImage() {
        ...
        try {
                ...
                const options = {
                        pathSimplificationTolerance: Number(controls.pathSimplification.value),
                ...
                if (options.pixelGridStepSize > options.minHullDistance) {
                        throw new Error('Pixel grid step size must not exceed the minimum hull distance.');
                }
                ...
                if (svgElement) {
                        svgElement.setAttribute('viewBox', `0 0 ${imageData.width} ${imageData.height}`);
                }
