...
export function smoothedSVGPathData(
	...
	let d: string[] = [];
	const arrayPoints = points.map(pt => [pt.x, pt.y]);
	// const cubics = FitCurve.fitCubic(
	// 	arrayPoints,
	// 	FitCurve.createTangent(arrayPoints[0], arrayPoints[1]),
	// 	FitCurve.createTangent(
	// 		arrayPoints[0],
	// 		arrayPoints[arrayPoints.length - 1]
	// 	),
	// 	1
	// );
	const cubics = FitCurve.default(arrayPoints, 1);
	d.push(`M ${nf(cubics[0][0][0])}`);
	return d.join(' ');
}
