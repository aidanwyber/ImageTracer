...
export class Hull {
        ...
        readonly sampledPoints: Point[];
        readonly hullPoints: Point[];
...
export class ImageTracer {
        ...
        constructor(imageData: ImageDataLike, palette: Color[], pixelGridSize = 1) {
                ...
                this.hulls = palette.map(color => this.createHullForColor(imageData, color));
        }

        getHullByColor(color: Color): Hull | undefined {
                return this.hulls.find(hull =>
                        hull.color.r === color.r && hull.color.g === color.g && hull.color.b === color.b
                );
        }
        ...
        private createMask(imageData: ImageDataLike, color: Color): { mask: Uint8ClampedArray; points: Point[] } {
                ...
                for (let y = 0; y < height; y++) {
                        for (let x = 0; x < width; x++) {
                                ...
                                const matches = a !== 0 && r === color.r && g === color.g && b === color.b;
